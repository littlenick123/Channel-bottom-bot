...
机器人监听已经绑定的 Telegram 频道和超级群组。出现新帖子后，它会等待默认 10 秒，将这段时间内的连发和相册事件合并，然后删除上一组置底帖，并按槽位编号从大到小重发。`1号`最后发送，因此位于聊天记录最底部。
...
- 每个频道/超级群组默认 10 个槽位，可独立启停、替换、清空，并支持总开关、静默发送、延迟和立即刷新。
...
> 传统基础群组（Telegram `group`）不受支持；请先将其升级为超级群组后再添加机器人。
...
- 大频道视频反复刷新或旧帖未删除：Telegram 可能先为自动排队的视频返回临时 `message_id=0`。新版会保存消息指纹，等视频真正发布后自动绑定实际消息 ID，再在下一次刷新时删除；数据库升级也会清除旧版留下的无效 ID。升级前已经发布且只记录为 `0` 的历史重复帖无法通过 Bot API 反查 ID，需要在频道中手动删除一次，之后的新置底帖会正常自动维护。
