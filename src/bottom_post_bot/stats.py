...
def _day_line(label: str, stats: DailyMemberStats) -> str:
    return f"{label}：新增 {stats.joined_count:,}｜退出 {stats.left_count:,}｜净增 {_signed(stats.net_change)}"
...
def format_chat_report(report: MemberStatsReport, *, timezone: ZoneInfo | str = "Asia/Shanghai") -> str:
    ...
    count = "暂不可用" if report.current_member_count is None else f"{report.current_member_count:,}"
    updated = "暂不可用"
    if report.current_count_at is not None:
        updated = datetime.fromtimestamp(report.current_count_at, tz).strftime("%Y-%m-%d %H:%M")
    push = "已开启" if report.stats_push_enabled else "已关闭"
    return "\n".join(
        (
            f"频道/群组：{title}",
            f"当前成员总数：{count}",
            _day_line("今天", report.today),
            _day_line("昨天", report.yesterday),
            f"更新时间：{updated}",
